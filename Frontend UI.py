...
with st.sidebar:
    ...
    if st.button("🏠 心情监测中心", use_container_width=True):
        st.session_state.page = 'home'

    if st.button("💝 情绪疗愈中心", use_container_width=True):
    ...
    # 定义HTML指示器
    indicator_html = '<span class="realtime-indicator"></span>'

    # 根据监测状态显示不同的文本
    if st.session_state.monitoring_active:
        status_text = f"监测状态: {indicator_html}运行中"
    else:
        status_text = "监测状态: 已暂停"
...
if st.session_state.page == 'home':
    ...
    with col1:
        ...
        # 模拟面部表情数据
        emotion_labels = ['开心', '平静', '焦虑', '悲伤', '愤怒']
        emotion_values = np.random.dirichlet(np.ones(5), size=1)[0]

        fig_emotion = go.Figure(data=[go.Pie(
            labels=emotion_labels,
            values=emotion_values,
            hole=0.4,
            marker_colors=['#4CAF50', '#2196F3', '#FF9800', '#9E9E9E', '#F44336']
        )])
        ...
        elif dominant_emotion in ['悲伤', '愤怒']:
    ...
    if not st.session_state.show_gad7:
        if st.button("开始焦虑评估", use_container_width=True):
        ...
        with chat_container:
            for message in st.session_state.chat_history:
                if message['role'] == 'user':
                    st.markdown(
                        f'<div class="chat-message user-message"><img src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=cartoon%20user%20avatar%20friendly%20face&image_size=square" class="avatar"><div class="message-content"><strong>你:</strong> {message["content"]}</div></div>',
                        unsafe_allow_html=True)
                else:
                    st.markdown(
                        f'<div class="chat-message bot-message"><img src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=cartoon%20robot%20avatar%20friendly%20blue%20color&image_size=square" class="avatar"><div class="message-content"><strong>小U:</strong> {message["content"]}</div></div>',
                        unsafe_allow_html=True)

        # 处理GAD-7问题
        if st.session_state.gad7_current_question < len(gad7_questions):
            # 如果是新问题，添加机器人的问题到聊天历史
            if len(st.session_state.chat_history) == 1 or st.session_state.chat_history[-1]['role'] == 'user':
            ...
            # 用户输入答案
            answer = st.text_input("输入你的答案(1-4):", key=f"gad7_input_{st.session_state.gad7_current_question}")

            if st.button("发送") and answer in ['1', '2', '3', '4']:
                # 映射答案到文本
                answer_map = {
                    '1': "完全不会",
                    '2': "好几天",
                ...
                st.session_state.chat_history.append({
                    'role': 'user',
                    'content': answer_map[answer],
                    'timestamp': datetime.now()
                })

                # 保存答案
                st.session_state.gad7_answers[st.session_state.gad7_current_question] = answer_map[answer]
            ...
            with col2:
                # 四项指标比例（模拟数据）
                facial_score = np.random.uniform(0, 100)  # 表情分析
                hrv_score = np.random.uniform(0, 100)     # HRV分析
                voice_score = np.random.uniform(0, 100)    # 语义分析（语音情绪）
                ...
                # 显示综合得分
                st.markdown(f"<div class='card' style='text-align: center;'><h4>综合评估得分</h4><h2>{weighted_score:.1f}</h2><p>权重: HRV 30% | 语义 25% | 表情 25% | 量表 20%</p></div>", unsafe_allow_html=True)

            # 综合建议
            st.markdown('<div class="card"><h4>💡 综合建议</h4></div>', unsafe_allow_html=True)
